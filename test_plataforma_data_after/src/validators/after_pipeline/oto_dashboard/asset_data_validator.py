...
class OtoAssetDataValidator(OtoDashboardSectionValidator):
    """Valida os totais numéricos expostos pelo objeto pai `asset_data`."""

    section_name = "asset_data"
    _CONFIGURATION_PATH = Path(__file__).with_name("asset_data_validation_controls.json")
    ...
    def validate(
        self,
        index_name: str,
        latest_asset_data: object,
        previous_asset_data: object,
        errors: list[str],
        details: list[str],
    ) -> None:
        if not isinstance(latest_asset_data, dict):
            errors.append(
                f"Índice '{index_name}' | seção **asset_data** ausente ou inválida "
                "no documento mais recente."
            )
            return
        if not isinstance(previous_asset_data, dict):
            errors.append(
                f"Índice '{index_name}' | seção **asset_data** ausente ou inválida "
        ...
        try:
            ...
            errors.append(
                f"Índice '{index_name}' | erro ao carregar controles de asset_data: "
                f"{error.__class__.__name__}: {error}"
            )
            return

        for field_name in value_fields:
            attribute_path = f"asset_data.{field_name}"
            latest_value = latest_asset_data.get(field_name)
            previous_value = previous_asset_data.get(field_name)
    ...
    def _validate_variation(
        ...
        message = (
            f"Índice '{index_name}' | seção **asset_data** | atributo **{attribute_path}**: "
