...
class OtoThreatIntelligenceDataValidator(OtoDashboardSectionValidator):
    """Valida os scores numéricos expostos por `threat_intelligence_data`."""

    section_name = "threat_intelligence_data"
    _CONFIGURATION_PATH = Path(
        __file__
    ).with_name("threat_intelligence_data_validation_controls.json")
    ...
    def validate(
        self,
        index_name: str,
        latest_threat_data: object,
        previous_threat_data: object,
        errors: list[str],
        details: list[str],
    ) -> None:
        if not isinstance(latest_threat_data, dict):
            errors.append(
                f"Índice '{index_name}' | seção **threat_intelligence_data** ausente ou "
                "inválida no documento mais recente."
            )
            return
        if not isinstance(previous_threat_data, dict):
            errors.append(
                f"Índice '{index_name}' | seção **threat_intelligence_data** ausente ou "
        ...
        try:
            ...
            errors.append(
                f"Índice '{index_name}' | erro ao carregar controles de "
                f"threat_intelligence_data: {error.__class__.__name__}: {error}"
            )
            return

        for field_name in value_fields:
            attribute_path = f"threat_intelligence_data.{field_name}"
            latest_value = latest_threat_data.get(field_name)
            previous_value = previous_threat_data.get(field_name)
    ...
    def _validate_variation(
        ...
        message = (
            f"Índice '{index_name}' | seção **threat_intelligence_data** | atributo "
